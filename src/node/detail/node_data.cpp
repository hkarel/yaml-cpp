...
// sequence
void node_data::push_back(const node_ptr& node) {
...
void node_data::insert(const node_ptr& key, const node_ptr& value) {
  switch (m_type) {
    case NodeType::Map:
      break;
    case NodeType::Undefined:
...
bool node_data::remove(const node_ptr& key) {
  if (m_type != NodeType::Map)
    return false;
  ...
  auto it =
      std::find_if(m_map.begin(), m_map.end(),
                   [&](std::pair<YAML::detail::node_ptr, YAML::detail::node_ptr> j) {
...
void node_data::insert_map_pair(const node_ptr& key, const node_ptr& value, bool force) {
